/*
 An example analogue meter using a ILI9163 TFT LCD screen
...
void analogMeter()
{
  ...
  // Meter outline
  tft.fillRect(0, 0, M_SIZE*239, M_SIZE*131, ILI9163_GREY);
  tft.fillRect(1, M_SIZE*3, M_SIZE*234, M_SIZE*125, ILI9163_WHITE);

  tft.setTextColor(ILI9163_BLACK);  // Text colour
  ...
  for (int i = -50; i < 51; i += 5) {
    ...
    // Yellow zone limits
    //if (i >= -50 && i < 0) {
    //  tft.fillTriangle(x0, y0, x1, y1, x2, y2, ILI9163_YELLOW);
    //  tft.fillTriangle(x1, y1, x2, y2, x3, y3, ILI9163_YELLOW);
    //}

    // Green zone limits
    if (i >= 0 && i < 25) {
      tft.fillTriangle(x0, y0, x1, y1, x2, y2, ILI9163_GREEN);
      tft.fillTriangle(x1, y1, x2, y2, x3, y3, ILI9163_GREEN);
    }

    // Orange zone limits
    if (i >= 25 && i < 50) {
      tft.fillTriangle(x0, y0, x1, y1, x2, y2, ILI9163_ORANGE);
      tft.fillTriangle(x1, y1, x2, y2, x3, y3, ILI9163_ORANGE);
    }
    ...
    // Draw tick
    tft.drawLine(x0, y0, x1, y1, ILI9163_BLACK);
    ...
    if (i < 50) tft.drawLine(x0, y0, x1, y1, ILI9163_BLACK);
  }
  ...
  tft.drawRect(1, M_SIZE*3, M_SIZE*236, M_SIZE*126, ILI9163_BLACK); // Draw bezel line
...
{
  tft.setTextColor(ILI9163_BLACK, ILI9163_WHITE);
  ...
  while (!(value == old_analog)) {
    ...
    tft.drawLine(M_SIZE*(120 + 24 * ltx) - 1, M_SIZE*(150 - 24), osx - 1, osy, ILI9163_WHITE);
    tft.drawLine(M_SIZE*(120 + 24 * ltx), M_SIZE*(150 - 24), osx, osy, ILI9163_WHITE);
    tft.drawLine(M_SIZE*(120 + 24 * ltx) + 1, M_SIZE*(150 - 24), osx + 1, osy, ILI9163_WHITE);
    ...
    tft.drawLine(M_SIZE*(120 + 24 * ltx) - 1, M_SIZE*(150 - 24), osx - 1, osy, ILI9163_RED);
    tft.drawLine(M_SIZE*(120 + 24 * ltx), M_SIZE*(150 - 24), osx, osy, ILI9163_MAGENTA);
    tft.drawLine(M_SIZE*(120 + 24 * ltx) + 1, M_SIZE*(150 - 24), osx + 1, osy, ILI9163_RED);
